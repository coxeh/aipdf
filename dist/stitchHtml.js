...
function validateStitchResponse(v) {
...
async function stitchOnePage(gemini, pageNum, userContext, primaryModel, fallbackModel) {
    const params = (model) => ({
        model,
    ...
    try {
        return await generateJsonWithRetry(gemini, `stitch:p${pageNum}`, params(primaryModel), { maxAttempts: 3, validate: (v) => validateStitchResponse(v) });
    }
    catch (err) {
        if (!fallbackModel)
            throw err;
        console.log(`[3/5]   page ${pageNum}: ${primaryModel} failed (${err.message.slice(0, 200)}). Falling back to ${fallbackModel}.`);
        return await generateJsonWithRetry(gemini, `stitch:p${pageNum}-fallback`, params(fallbackModel), { maxAttempts: 3, validate: (v) => validateStitchResponse(v) });
    }
}
export async function stitchHtml(htmlPaths, outPath, statePath, opts) {
    const { gemini, force = false, tailChars = DEFAULT_TAIL_CHARS, boilerplateMax = DEFAULT_BOILERPLATE_MAX, model = DEFAULT_MODEL, fallbackModel = DEFAULT_FALLBACK_MODEL, } = opts;
    const effectiveFallback = fallbackModel && fallbackModel.length > 0 ? fallbackModel : null;
    if (force) {
        await writeFile(statePath, JSON.stringify({ pagesProcessed: 0, stitched: "", headerExamples: [], footerExamples: [] }, null, 2));
    }
    else {
        try {
    ...
    console.log(`[3/5] Stitching ${total} page(s) incrementally (model=${model}, fallback=${effectiveFallback ?? "none"}, resuming from page ${state.pagesProcessed + 1}).`);
    for (let i = state.pagesProcessed; i < total; i++) {
        ...
        const parsed = await stitchOnePage(gemini, i + 1, userContext, model, effectiveFallback);
        // Apply the diff: replace tail if requested, then append.
        const base = parsed.tailReplacement !== null
        ...
        console.log(`[3/5]   stitched page ${i + 1}/${total} (${mergeKind}; doc=${state.stitched.length} chars, headers=${state.headerExamples.length}, footers=${state.footerExamples.length})`);
    }
    return state.stitched;
}
//# sourceMappingURL=stitchHtml.js.map