...
async function ocrSinglePage(gemini, task) {
    const pageNum = task.idx + 1;
    const data = (await readFile(task.imgPath)).toString("base64");
    const parsed = await generateJsonWithRetry(gemini, `ocr:p${pageNum}`, {
        model: gemini.model,
        ...
        maxAttempts: 3,
        validate: (v) => {
            if (!v || typeof v !== "object" || typeof v.html !== "string") {
...
export async function ocrPages(imagePaths, outDir, opts) {
    ...
    async function worker(workerId) {
        while (true) {
            ...
            try {
                ...
                completed++;
                console.log(`[2/5]   (${completed}/${total}) [w${workerId}] done page ${task.idx + 1}`);
            }
            catch (err) {
                console.error(`[2/5]   [w${workerId}] error on page ${task.idx + 1}: ${err.message}`);
                throw err;
            }
        }
    }
...
//# sourceMappingURL=ocrToHtml.js.map