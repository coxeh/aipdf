...
async function recommendByDataVolume(structured, opts) {
    ...
    console.log(`[5/5] Deterministic pick: ${chosen.name} (${chosen.records.length} records × ${Object.keys(chosen.schema).length} fields = ${cellCount(chosen)} cells; runner-up: ${runnerUp ?? "(none)"}).`);
    const narrative = await generateJsonWithRetry(opts.gemini, "recommend-narrative", {
...
async function recommendByLlmJudgement(structured, opts) {
    ...
    const result = await generateJsonWithRetry(opts.gemini, "recommend", {
        model: opts.gemini.model,
        config: { responseMimeType: "application/json" },
        contents: [
            {
...
export async function recommendProminent(structured, outPath, opts) {
    ...
    const recommendation = strategy === "data-volume"
