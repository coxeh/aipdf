...
function dedupePush(list, value, max) {
    ...
    list.push(v);
    if (list.length > max)
        list.shift(); // evict least-recently-seen
}
...
export async function stitchHtml(htmlPaths, outPath, statePath, opts) {
    const { gemini, force = false, tailChars = DEFAULT_TAIL_CHARS, boilerplateMax = DEFAULT_BOILERPLATE_MAX, } = opts;
    if (force) {
    ...
    for (let i = state.pagesProcessed; i < total; i++) {
        ...
        const parsed = await generateJsonWithRetry(gemini, `stitch:p${i + 1}`, {
            model: gemini.model,
            ...
            maxAttempts: 3,
            validate: (v) => {
                if (!v || typeof v !== "object" || typeof v.merged !== "string") {
        ...
        console.log(`[3/5]   stitched page ${i + 1}/${total} (doc=${state.stitched.length} chars, headers=${state.headerExamples.length}, footers=${state.footerExamples.length})`);
    }
