...
import { loadFieldsFile, parseFieldsFlag, processPdf, type PreferredField } from "./lib";
...
async function main() {
  ...
  let preferredFields: PreferredField[] | undefined;
  if (args.fieldsFile) {
  ...
  if (result.fieldMapping) {
    const matched = result.fieldMapping.mappings.reduce(
  ...
  if (result.addressAnalysis) {
    ...
    for (const rec of result.addressAnalysis.addressRecommendations) {
      ...
      console.log(
        `  - ${rec.groupName}: [${rec.columns.join(", ")}]  template: ${rec.joinTemplate}  (${rec.confidence})`,
      );
      if (rec.exampleJoined) console.log(`      example: ${rec.exampleJoined}`);
    }
  }
  ...
  console.log(
    `  total records:       ${result.merged.groups.reduce((n, g) => n + g.records.length, 0)}`,
  );
  ...
  if (Object.keys(c.byStage).length) {
    ...
    for (const [stage, s] of Object.entries(c.byStage)) {
      console.log(
        `    ${stage.padEnd(20)} ${String(s.calls).padStart(4)} call(s)  $${s.costUsd.toFixed(4)}`,
      );
    }
  }
  if (c.unknownPricingModels.length) {
    console.log(
      `  WARNING: no pricing for model(s) ${c.unknownPricingModels.join(", ")} - cost shown as $0 for those calls.`,
    );
  }
