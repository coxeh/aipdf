...
import { runExtractor, type SandboxOptions } from "./sandbox";
...
function truncateRegion(region: string, max: number): string {
  if (region.length <= max) return region;
 ...
 */
export function buildExtractorSample(
  html: string,
  opts: ExtractorSampleOptions = {},
): { sample: string; sampledChars: number; totalChars: number; regionsIncluded: number } {
  ...
  const regions: Array<{
    start: { title: string; kind: string };
    from: number;
    to: number;
  }> = [];
  ...
  if (regions.length === 0) {
    ...
    const sample =
      `<!-- SAMPLE: NO STRUCTURED ANCHORS DETECTED -->\n` +
  ...
  for (const r of regions) {
    ...
    parts.push(
      `<!-- SAMPLE: STRUCTURED REGION title="${r.start.title}" kind="${r.start.kind}" (first occurrence) -->`,
      region,
    );
  ...
  if (tail > 0) {
    ...
    parts.push(
      `<!-- SAMPLE: DOCUMENT TAIL (last ${tailSlice.length} chars of ${total}) -->`,
      tailSlice,
    );
  }
...
async function tryRunExtractor(
  code: string,
  stitchedHtml: string,
  sandbox: SandboxOptions | undefined,
): Promise<StructuredOutput> {
  const result = await runExtractor(code, stitchedHtml, sandbox ?? {});
  if (!result || typeof result !== "object" || !Array.isArray((result as StructuredOutput).groups)) {
    throw new Error(
      `extractor did not return { groups: [...] }; got: ${JSON.stringify(result)?.slice(0, 200)}`,
    );
  }
  return result as StructuredOutput;
}
...
export async function extractStructured(
  ...
  const { sample, sampledChars, totalChars, regionsIncluded } = buildExtractorSample(
    stitchedHtml,
    sampleOpts,
  );
  if (sampledChars < totalChars) {
    console.log(
      `[4/5] Sampling HTML for script generation: ${sampledChars.toLocaleString()} of ${totalChars.toLocaleString()} chars (${regionsIncluded} region(s) included).`,
    );
  } else {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (isFirst) {
      ...
      console.log(
        `[4/5] Asking Gemini to FIX the extractor (attempt ${attempt}/${maxAttempts}) after error: ${lastError!.message.slice(0, 200)}`,
      );
    }
    ...
    try {
      ...
      console.log(
        `[4/5] Success on attempt ${attempt}. Found ${result.groups.length} group(s); ${result.groups.reduce((n, g) => n + g.records.length, 0)} record(s) total.`,
      );
      return result;
    } catch (err) {
  ...
  throw new Error(
    `Extractor failed after ${maxAttempts} attempts. Last error: ${lastError?.message ?? "unknown"}\nLast script saved at: ${scriptPath}`,
  );
}
