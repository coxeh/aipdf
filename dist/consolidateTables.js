...
function applyConsolidations(structured, analysis) {
    ...
    for (const c of analysis.consolidations ?? []) {
        if (c.kind === "row-merge") {
            const srcGroups = c.groups
                ...
                .filter((g) => !!g);
            if (srcGroups.length < 2)
                continue;
            const schema = { ...c.unifiedSchema };
            const records = [];
            ...
            const schema = { ...c.unifiedSchema };
            if (!schema[c.targetField])
                schema[c.targetField] = src.schema[pivot[0]] ?? "string";
            for (const k of keep) {
                if (!schema[k])
                    schema[k] = src.schema[k] ?? "string";
            }
            newGroups.push({
                name: c.consolidatedName,
                title: c.consolidatedTitle,
    ...
    for (const g of structured.groups) {
        if (!consumed.has(g.name))
            newGroups.push(g);
    }
    return { groups: newGroups };
}
function detectAdjacentTables(html, maxGapChars = 800) {
    const { starts, ends } = parseAnchors(html);
    const hints = [];
    for (let i = 0; i < starts.length - 1; i++) {
        const s = starts[i];
        const next = starts[i + 1];
        if (s.kind !== "table" || next.kind !== "table")
            continue;
        ...
        if (!gapStripped)
            continue;
        if (gapStripped.length > maxGapChars)
            continue;
        hints.push({
            firstTitle: s.title,
            firstKind: s.kind,
...
export async function consolidateTables(structured, analysisPath, consolidatedPath, opts) {
    ...
    if (!Array.isArray(analysis.consolidations))
    ...
    console.log(`[4d/5] Applied ${rowMerges} row-merge(s) and ${pivots} column-pivot(s); ${structured.groups.length} -> ${consolidated.groups.length} group(s).`);
    return { analysis, consolidated };
}
//# sourceMappingURL=consolidateTables.js.map