...
import { stripFences, type GeminiClient } from "./gemini";
import { runTextPatternExtractor, type SandboxOptions } from "./sandbox";
...
export function htmlToPlainText(html: string): string {
  ...
  const root = $.root()[0] as unknown as DomNode | undefined;
  if (!root) return "";
...
export function samplePlainText(
  text: string,
  opts: { headChars?: number; tailChars?: number; maxChars?: number } = {},
): { sample: string; sampledChars: number; totalChars: number } {
...
export async function proposePatterns(
  sample: string,
  outPath: string,
  opts: DiscoverPatternsOptions,
): Promise<PatternsProposal> {
  ...
  if (!force) {
    try {
      ...
      if (s.size > 0) {
        ...
        return JSON.parse(await readFile(outPath, "utf8")) as PatternsProposal;
      }
    } catch {}
  }
  ...
  let parsed: PatternsProposal;
  try {
    parsed = JSON.parse(res.text ?? "{}") as PatternsProposal;
  } catch (err) {
    throw new Error(`Failed to parse patterns JSON: ${(err as Error).message}`);
  }
...
export async function extractWithPatterns(
  ...
  if (!force) {
    try {
      ...
      if (s.size > 0) {
        ...
        return JSON.parse(await readFile(outJsonPath, "utf8")) as StructuredOutput;
      }
    } catch {}
  }
  ...
  if (patterns.patterns.length === 0) {
    ...
    const empty: StructuredOutput = { groups: [] };
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (isFirst) {
      ...
      console.log(
        `[4b/5] Asking Gemini to FIX the pattern extractor (attempt ${attempt}/${maxAttempts}) after error: ${lastError!.message.slice(0, 200)}`,
      );
    }
    ...
    if (!hasExtractFunction(code)) {
      lastError = new Error(
        `generated script does not declare an "extract" function (function declaration or const/let assignment). First 500 chars:\n${code.slice(0, 500)}`,
      );
    ...
    try {
      ...
      if (!raw || typeof raw !== "object" || !Array.isArray((raw as StructuredOutput).groups)) {
        throw new Error(
          `extractor did not return { groups: [...] }; got: ${JSON.stringify(raw)?.slice(0, 200)}`,
        );
      }
      const typed = raw as StructuredOutput;
      await writeFile(outJsonPath, JSON.stringify(typed, null, 2));
      console.log(
        `[4b/5] Success on attempt ${attempt}. Found ${typed.groups.length} group(s); ${typed.groups.reduce((n, g) => n + g.records.length, 0)} record(s) total.`,
      );
      return typed;
    } catch (err) {
  ...
  throw new Error(
    `Pattern extractor failed after ${maxAttempts} attempts. Last error: ${lastError?.message ?? "unknown"}`,
  );
}
...
export async function discoverStructuredFromText(
  stitchedHtml: string,
  paths: DiscoverPaths,
  opts: DiscoverPatternsOptions,
): Promise<{ proposal: PatternsProposal; structured: StructuredOutput; plainText: string }> {
  ...
  if (sampledChars < totalChars) {
    console.log(
      `[4b/5] Sampling text: ${sampledChars.toLocaleString()} of ${totalChars.toLocaleString()} chars.`,
    );
  }

  const proposal = await proposePatterns(sample, paths.patternsPath, opts);
  const structured = await extractWithPatterns(
    plain,
    sample,
    proposal,
    paths.outJsonPath,
