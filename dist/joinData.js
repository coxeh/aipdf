...
function applyMerges(structured, analysis) {
    ...
    for (const merge of analysis.merges ?? []) {
        ...
        for (const src of srcGroups) {
            ...
            for (const [srcField, dstField] of Object.entries(map)) {
                if (!schema[dstField])
                    schema[dstField] = src.schema[srcField] ?? "string";
            }
            for (const [field, type] of Object.entries(src.schema)) {
                if (!(field in map) && !(field in schema))
                    schema[field] = type;
            }
        }
        const records = [];
...
export async function analyzeJoins(structured, analysisPath, mergedPath, opts) {
    ...
    console.log(`[4.5/5] Found ${analysis.merges.length} merge(s) and ${analysis.joins.length} join(s); merged output has ${merged.groups.length} group(s).`);
    return { analysis, merged };
}
//# sourceMappingURL=joinData.js.map