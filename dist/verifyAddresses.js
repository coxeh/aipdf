...
function pickNameField(schema, override) {
    if (override && override in schema)
        return override;
    for (const f of NAME_FIELD_CANDIDATES)
        if (f in schema)
            return f;
    return null;
}
function buildPrompt(nameField, addressField, documentTitle, documentSummary) {
    const contextLines = [];
    if (documentTitle)
        contextLines.push(`- Title: ${documentTitle}`);
    if (documentSummary)
        contextLines.push(`- Summary: ${documentSummary}`);
...
export async function verifyAddressesAndNames(final, outPath, opts) {
    const { gemini, force = false, batchSize = 5, useGrounding = true, concurrency = 4, } = opts;
    if (!force) {
        try {
    ...
    console.log(`[verify] Verifying ${total} record(s) in ${numBatches} batch(es) of ${batchSize}, concurrency=${effConcurrency}${grounded ? ", Google Search grounding ON" : ""}.`);
    console.log(`[verify]   name field: ${nameField ?? "(none — using all sample fields)"}; address field: ${addressField}`);
    ...
    async function runBatch(batchIdx) {
        ...
        const inputs = batch.map((r, j) => {
            const o = { index: start + j };
            if (nameField)
                o.name = r[nameField];
            o.address = r[addressField];
            for (const col of final.address.columns)
                o[col] = r[col];
            return o;
        });
        const parsed = await generateJsonWithRetry(gemini, `verify:b${batchNum}`, {
            model: gemini.model,
            ...(config ? { config } : {}),
            contents: [
                {
        ...
        for (const v of parsed.verifications) {
            ...
            const confidence = c === "high" || c === "medium" || c === "low" ? c : "unknown";
            byIdx.set(v.index, {
                confidence,
        ...
        for (let j = 0; j < batch.length; j++) {
            const idx = start + j;
            const verification = byIdx.get(idx) ?? {
                confidence: "unknown",
                matchedName: null,
                matchedAddress: null,
    ...
    const verifyStages = Object.entries(allCosts.byStage).filter(([k]) => k.startsWith("verify:"));
    ...
    console.log(`[verify] Done. high=${summary.high} medium=${summary.medium} low=${summary.low} unknown=${summary.unknown}`);
    console.log(`[verify] Cost: tokens=$${tokensCostUsd.toFixed(4)} (${inputTokens.toLocaleString()} in / ${outputTokens.toLocaleString()} out)` +
...
export async function verifyAddressesFromFile(finalJsonPath, outPath, opts) {
    const final = JSON.parse(await readFile(finalJsonPath, "utf8"));
...
//# sourceMappingURL=verifyAddresses.js.map